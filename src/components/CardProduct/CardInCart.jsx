...
const CardInCart = (props) => {
    ...
    const [countProduct, setCountProduct] = React.useState(props.dataProduct?.quantity ?? 0)
    const [sizeProduct, setSizeProduct] = React.useState(props.dataProduct?.size)
      ...
      <Card sx={{ display: "flex", margin: '15px 0'}}>
        ...
        <Box sx={{ width: "76%" }}>
          <Box sx={{ display: "flex", flexDirection: "row" }}>
            <Box sx={{ padding: "8px 0 0 20px", flexGrow: "1", width: 100 }}>
              <Typography
                sx={{
                  display: "block",
                  fontSize: "16px",
              ...
              >
                {productInCart?.price?.toLocaleString('it-IT') ?? 'price'}
                <span> <b> <u>đ</u></b></span>
              </Typography>
              <div className="form-size-product">
                <label htmlFor="sizeProduct">Kích cỡ: </label>
                <select id="sizeProduct" value={sizeProduct} onChange={e => handleSelectSize(e)}>
            ...
            <IconButton aria-label="delete" onClick={() => handleDeleteInCart()}>
              <DeleteIcon />
            </IconButton>
          </Box>
          <Box
          ...
          >
            <div className="cart-update-number">
              <button onClick={() => {setCountProduct(countProduct > 1 ? parseInt(countProduct) - 1: 1)}}>-</button>
              <input type="text" value={countProduct} readOnly />
              <button onClick={() => setCountProduct(parseInt(countProduct) + 1)}>+</button>
            </div>
            <Typography sx={{ fontSize: "14px" }}>
              ...
              >
                {(parseInt(productInCart?.price) * parseInt(countProduct))?.toLocaleString('it-IT')}
              </span>
              <span> <b><u>đ</u></b></span>
            </Typography>
          </Box>
