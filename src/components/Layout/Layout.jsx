  ...
  return (
    <>
      <header className="page-header">
        ...
        >
          <ul className="navbar">
            {navbarData && navbarData.map((navbarItem, index) => (
              <ItemNav key={index} {...navbarItem} />
            ))}
          </ul>
          <ul className="header-toolbar">
            <li className="form-search">
              <input type="text" placeholder="Tìm kiếm..." value={textSearch} onChange={e => setTextSearch(e.target.value)} />
            ...
            <li className="header-toolbar__item header-cart">
                <a href="/cart">
                    <button className="header-button" title="Giỏ hàng">
              ...
              <div className="toolbar-cart">
                ...
                <Box sx={{margin: '20px 10px 0', padding: '8px 12px', borderTop: '1px solid #BCBCBC'}}>
                    <Typography sx={{ fontSize: "16px", textAlign: 'end' }}>
                        ...
                        >
                            {sumPriceProduct?.toLocaleString('it-IT')}
                        </span>
                        <span> <b> <u>đ</u></b></span>
                    </Typography>
                    <a href="/cart">
                      <Button variant="contained" sx={{fontSize: '15px', height: '35px', margin: '10px auto 5px', display: 'block'}}>Xem giỏ hàng</Button>
                    </a>
                </Box>
              </div>
            </li>
            <li className="header-toolbar__item">
              ...
              <div className="toolbar-menu">
                ...
                <ul hidden={!isLogin}>
                  ...
                  <li>
                    <Button
                      sx={{ display: "block", color: "inherit", padding: "0" }}
            ...
            <div className="subnav-left">
              {options &&
                options.map((option, i) => (
                  <ul key={i}>
                    <li>
                      <a href={`/${link}/${option.link}`}>{option.label}</a>
                    </li>
                    {option.options &&
                      option.options.map((item, key) => (
                        <li key={key}>
                          <a href={`/${link}/${item.link}`}>{item.label}</a>
                        </li>
                      ))}
                  </ul>
                ))}
