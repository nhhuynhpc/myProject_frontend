...
// service
import { GetProduct, getProductDetail } from '../services/productService';
import CardProduct from '../components/CardProduct/CardProduct';
import { Link, useNavigate, useParams } from 'react-router-dom';
import Add from '@mui/icons-material/Add';
import RemoveIcon from '@mui/icons-material/Remove';
import { useDispatch, useSelector } from 'react-redux';
import { addCart, updateCart } from '../redux/slides/cartSlide';
...
const ProductDetail = () => {
    ...
    const getProductDetailData = async () => {
        try {
            let result = await getProductDetail({
                    ...
                    <Breadcrumbs aria-label="breadcrumb">
                        ...
                        <Link to={'/'}>
                            <span style={{ color: '#959598' }}>
                                {dataProductDetail?.typeCustomer ?? 'err'}
                            </span>
                        </Link>
                        <Link to={'/'}>
                            <span style={{ color: '#959598' }}>
                                {dataProductDetail?.typeProduct ?? 'err'}
                            </span>
                        </Link>
                        <span style={{ color: '#1C1F1F' }}>
                            {dataProductDetail?.typeProductDetail ?? 'err'}
                        </span>
                    </Breadcrumbs>
                </Box>
                <Box
                ...
                >
                        ...
                        <img
                            src={
                                dataProductDetail?.image
                                    ? api_url + dataProductDetail.image
                                    : ''
                            }
                            alt="img product"
                        />
                    ...
                    >
                        ...
                        <span className="price">
                            {dataProductDetail?.price?.toLocaleString('it-IT')}
                        </span>
                        <span>
                            {' '}
                            <b>
                        ...
                        >
                            <Button onClick={() => setProductSize('S')}>
                                S
                            </Button>
                            <Button onClick={() => setProductSize('M')}>
                                M
                            </Button>
                            <Button onClick={() => setProductSize('L')}>
                                L
                            </Button>
                            <Button onClick={() => setProductSize('XL')}>
                                XL
                            </Button>
                        </ButtonGroup>
                        <Typography sx={{ margin: '20px 0 0' }}>
                            Số lượng:{' '}
                        </Typography>
                        <Box className="form-number-product">
                            <button
                                onClick={() =>
                                    setValueProduct(
                                        parseInt(valueProduct) - 1 < 1
                                            ? 1
                                            : parseInt(valueProduct) - 1
                                    )
                                }
                            >
                                <RemoveIcon />
                            ...
                            <button
                                onClick={() =>
                                    setValueProduct(parseInt(valueProduct) + 1)
                                }
                            >
                                <Add />
                            </button>
                        ...
                        >
                            ...
                            <AccordionDetails>
                                <Typography>
                                    {dataProductDetail?.description ?? 'err'}
                                </Typography>
                            </AccordionDetails>
                        </Accordion>
                        <Accordion
