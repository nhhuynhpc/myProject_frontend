...
import { GetListCategories, GetProduct, PostDeleteCategories, PostListCategories, PostUpdateCategories, Postimg, PostimgCate } from '../../../services/authService'
...
const Categories = () => {
  ...
  const handleDeleteCate = async (id) => {
    let result = await PostDeleteCategories({id: id})
  ...
  const ItemCate = (item) => {
    return (
      <React.Fragment>
       ...
       <td>{item.id}</td>
       <td>{item.name}</td>
       <td>{item?.image ? <img src={API + item.image} alt="" width={150} height={150} />: 'Null'}</td>
       <td>{item.type}</td>
       <td>
  ...
  return (
    <>
      <AddCate
        ...
        postData={typeEdit === 0 ? handleSubcribeAddData: handleEditCate}
      />
      <div className="adminpage-container">
        ...
        <div className="categories-content">
          <Button className='add-data' dataValue="Thêm loại sản phẩm" onClick={ () => handleChangeData(0) } />
